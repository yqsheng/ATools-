	...
	: QDialog(mainWindow)
{
	...
	label->setText("ATools v" % string::number(VERSION) % '.' % string::number(SUB_VERSION) % " - " % tr("Par") % " syq"
		% "<br/>Open Source"
		% "<br/>Made in France"
		% "<br/>extend by syq"
		% "<br/>" % tr("Splashscreen et logo par") % " Adler"
		% "<br/>" % tr("Icônes de") % " <a href=\"http://icons8.com/\">Icons8</a>"
		% "<br/><br/><a href=\"https://github.com/yaqisheng/ATools-\">" % tr("Contributez sur") % " GitHub</a>");

...
void CMainFrame::_connectWidgets()
{
	...
	connect(ui.actionGetRelyOn, SIGNAL(triggered()), this, SLOT(showdds()));
}
void CMainFrame::showdds()
{
	dds = new getdds;
	dds->setWindowTitle("模型清理工具");
	dds->setWindowIcon(QIcon(":/MainFrame/Resources/ModelEditor.png"));
	dds->exec();
}
...
void CMainFrame::OpenFile(const string& filename)
{
	...
	if (GetExtension(filenameToLower) == "o3d")
	{
		...
		if (m_mesh && (filenameToLower.startsWith("part") || filenameToLower.startsWith("mvr") || filenameToLower.startsWith("item")) && m_motionList->rowCount() == 0)
		{
			...
			//有性别的物品的动作，是公用同一套 动作  类似mvr_male_xxx.ani 
			//比如怪物文件的话，才是有自己的动作
			//这是对上面的判断的解释
